...
int parse_and_check_file(char* file, char* symbols, int* parts_number_sum);
...
int main(void){
  int sum;
  parse_and_check_file("input.txt", "*%/#$+=@&%()!:><_-\\|", &sum);
  printf("SUM = %d\n", sum);
  return 0;
}
  ...
  *parts_number_sum = 0;
  ...
  while(has_reached_end != 1){
    ...
    while (symbol != NULL){
      ...
      //Top 3
      if(get_number_in_string_from_position(buffer_prev, pos - 1, &temp) == 0) *parts_number_sum += temp;
      prev_temp = temp;
      if((get_number_in_string_from_position(buffer_prev, pos, &temp) == 0) && temp != prev_temp) *parts_number_sum += temp;
      prev_temp = temp;
      if((get_number_in_string_from_position(buffer_prev, pos + 1, &temp) == 0) && temp != prev_temp) *parts_number_sum += temp;
      //Middle 2
      if(get_number_in_string_from_position(buffer, pos - 1, &temp) == 0) *parts_number_sum += temp;
      if(get_number_in_string_from_position(buffer, pos + 1, &temp) == 0) *parts_number_sum += temp;
      //Bottom 3
      if(has_reached_end == 0){
        if(get_number_in_string_from_position(buffer_next, pos - 1, &temp) == 0) *parts_number_sum += temp;
        prev_temp = temp;
        if((get_number_in_string_from_position(buffer_next, pos, &temp) == 0) && temp != prev_temp) *parts_number_sum += temp;
        prev_temp = temp;
        if((get_number_in_string_from_position(buffer_next, pos + 1, &temp) == 0) && temp != prev_temp) *parts_number_sum += temp;
      }
